...
const Attachment = () => {
  ...
  useEffect(() => {
    ...
    const fetchPatientData = async () => {
      try {
        const response = await axios.get(`https://hmsbackend-wbfe.onrender.com/api/v1/combined-data/${id}`);
        setPatientData(response.data);
        setPatientId(response.data.patientId)
        console.log("pid"+patientId)
      } catch (error) {
  ...
  const handleFileUpload = async (index, file) => {
    ...
    try {
      const response = await axios.post('https://hmsbackend-wbfe.onrender.com/uploads/upload', formData);
  ...
  const handleSaveFile = async (index) => {
    try {
      const response = await axios.post(`https://hmsbackend-wbfe.onrender.com/uploads/save`, {
        index,
        filename: uploadedFilename,
        patientId,
      });
  ...
  return (
    ...
    <div className='backk'>
    <Link to="/Patient">
          <FaArrowCircleLeft  style={{fontSize:'40px', color:'white'}}/>
        </Link>
      <h1 className='attachmentsheading23'>
