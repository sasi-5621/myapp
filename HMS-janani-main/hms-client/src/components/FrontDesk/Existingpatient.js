...
const ExistingPatient = () => {
  ...
  const handleSearch = () => {
    fetch(`https://hmsbackend-wbfe.onrender.com/api/v1/combined-data?searchTerm=${searchTerm}`)
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://hmsbackend-wbfe.onrender.com/api/v1/existingpatients-data",
        requestData
      );
  ...
  const isAdditionalFormValid =
    ...
    useEffect(() => {
      fetch("https://hmsbackend-wbfe.onrender.com/api/v1/combined-data")
        .then((response) => response.json())
        .then((data) => {
          setCurrentData(data);
          console.log("Fetched Data:", data);
    ...
    const fetchDoctors = async () => {
      try {
        const response = await axios.get(
          "https://hmsbackend-wbfe.onrender.com/api/doctors"
        );
